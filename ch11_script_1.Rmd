...
```{r}
county %>% summarize(cor = cor(poverty, less_than_hs))
```
...
p <- county %>%
  ...
  ggplot(aes(poverty, less_than_hs, color = region)) + geom_point() +
...
```{r}
anim <- p + 
  transition_manual(frames = region) + 
  shadow_mark() + 
  enter_grow() + 
  enter_fade()
...
```{r}
model <- lm(formula = poverty ~ less_than_hs, data = county)
